...
static void* fail_malloc(size_t size) { (void)size; return 0; }
static void* fail_realloc(void* pointer, size_t size) { (void)pointer; (void)size; return 0; }
...
TEST(AllocationFailure, StillCompletes)
{
    EXPECT_EQ(1, 2);
}
...
int main(int argc, char** argv)
{
    ...
    if (result != 1)
        return 1;
    if (strstr(g_output, "1 failure detail(s) could not be retained") == 0)
        return 2;
    return 0;
}
